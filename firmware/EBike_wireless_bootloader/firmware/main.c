...
static __attribute__((optimize("O0"))) bool read_pin(uint32_t pin)
{
  ...
  // don´t know why but next repetitive lines are needed so bootloader does not incorrectly detect UP and DOWN button presses at startup, at least on BAfang M500/M600 display hardware
